...
public class GamesEndpointsTests
{
    private record GameResponse(
        ...
        string? Barcode, string? IgdbId, string? ImagePath, string? Notes,
        DateTime AddedAt, DateTime UpdatedAt);

    private static object SampleDto(
        string title = "Hades",
        string? platform = "PC",
        bool isDigital = true,
        DigitalStore? store = DigitalStore.Steam) => new
        {
            Title = title,
            Platform = platform,
            Year = (int?)2020,
    ...
    public async Task Create_Unauthenticated_ReturnsUnauthorized()
    {
        ...
        var response = await client.PostAsJsonAsync("/api/games/", SampleDto());
    ...
    public async Task Create_AsAuthenticatedUser_Returns201WithBody()
    {
        ...
        var response = await alice.Client.PostAsJsonAsync("/api/games/", SampleDto());
    ...
    public async Task Create_PersistsOwnerIdFromAuthenticatedUser()
    {
        ...
        var created = await (await alice.Client.PostAsJsonAsync("/api/games/", SampleDto()))
    ...
    public async Task Create_WithEmptyTitle_ReturnsBadRequest()
    {
        ...
        var response = await alice.Client.PostAsJsonAsync("/api/games/", SampleDto(title: ""));
